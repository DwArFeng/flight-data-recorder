...
public class PersistenceValueCacheDelegate {
    ...
    @Transactional(transactionManager = "hibernateTransactionManager", readOnly = true)
    @TimeAnalyse
    public PersistenceValue get(@NotNull GuidKey key) throws CacheException {
        try {
    ...
    @TimeAnalyse
    public void push(@NotNull GuidKey key, @NotNull PersistenceValue persistenceValue, @Min(0) long timeout) throws CacheException {
        try {
    ...
    public void deleteAll(GuidKey pointKey) throws CacheException {
        try {
            ...
            for (String key : keys) {
                ...
                if (Objects.equals(redisGuidKey, redisPersistenceValue.getPointKey())) {
                    template.delete(key);
                }
            }
        } catch (Exception e) {
