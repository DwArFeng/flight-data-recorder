package com.dwarfeng.fdr.sdk.validate;
...
public @interface SpecificClass {
    ...
    class SpecificClassConstraintValidator implements ConstraintValidator<SpecificClass, Object> {
        ...
        public void initialize(SpecificClass constraintAnnotation) {
            LOGGER.debug("SpecificClassConstraintValidator: 初始化...");
            LOGGER.debug("SpecificClassConstraintValidator: 获取注解的 requiredClass 属性，为: " + constraintAnnotation.requiredClass().getCanonicalName() + "...");
            requiredClass = constraintAnnotation.requiredClass();
            LOGGER.debug("SpecificClassConstraintValidator: 获取注解的 allowSubClass 属性，为: " + constraintAnnotation.allowSubClass() + "...");
        ...
        public boolean isValid(Object value, ConstraintValidatorContext context) {
            LOGGER.debug("SpecificClassConstraintValidator: 开始验证...");
            try {
                if (allowSubClass) {
                    LOGGER.debug(
                            "SpecificClassConstraintValidator: 允许验证对象为指定类的子类，判断验证对象的类是否是 "
                                    + requiredClass.getCanonicalName() + " 的子类或本身类..."
                    );
                    boolean aFlag = requiredClass.isAssignableFrom(value.getClass());
                    LOGGER.debug(
                            "SpecificClassConstraintValidator: 验证结束，判断验证对象的类是否是 "
                                    + requiredClass.getCanonicalName() + " 的子类或本身类，结果为 " + aFlag + "..."
                    );
                    return aFlag;
                } else {
                    LOGGER.debug(
                            "SpecificClassConstraintValidator: 不允许验证对象为指定类的子类，判断验证对象的类与 "
                                    + requiredClass.getCanonicalName() + " 是否相等..."
                    );
                    boolean aFlag = Objects.equals(value.getClass(), requiredClass);
                    LOGGER.debug(
                            "SpecificClassConstraintValidator: 验证结束，判断验证对象的类与 "
                                    + requiredClass.getCanonicalName() + " 是否相等，结果为 " + aFlag + "..."
                    );
                    return aFlag;
                }
            } catch (Exception e) {
                LOGGER.warn("SpecificClassConstraintValidator: 验证过程中发生了意料之外的异常", e);
                return false;
            }
        }
    }
